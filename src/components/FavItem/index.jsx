...
export default function FavItem({ single }) {
  ...
  const {favList,setFavList,showNotification} = useContext(MovieContext);

  const handleRemove = () => {
    const updatedFavList = favList.filter(movie => movie.id !== single.id);
    setFavList(updatedFavList);
    showNotification(`${single.title} has been removed from favorites.`);
  }
  ...
  return (
    <>
    <div className="chakra-petch-regular flex items-start mb-4 mt-10">
